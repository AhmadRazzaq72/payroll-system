...
import { FaDownload, FaEnvelope, FaPhone, FaGlobe, FaCalendarAlt, FaDollarSign, FaRegClock } from "react-icons/fa";
...
function InfoTab() {
  ...
  const calculateNetSalary = () => {
    ...
    const tax = base * (Number(employee.tax_percent || 0) / 100);
    const pf = base * (Number(employee.pf_percent || 0) / 100);
    return (base - tax - pf).toFixed(2);
  };
        ...
        <>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
              ...
              <p><span className="font-medium">Paid Leaves/Month:</span> {employee.paid_leaves_allowed}</p>
            </div>
          </div>
      ...
      <div className="bg-white max-w-2xl w-full rounded-xl shadow-xl p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-sm">
          <div className="grid grid-cols-2 gap-3">
            ...
            <input type="number" name="paid_leaves_allowed" value={formData.paid_leaves_allowed} onChange={handleChange} placeholder="Paid Leaves/Month" className="border p-2 rounded" />
