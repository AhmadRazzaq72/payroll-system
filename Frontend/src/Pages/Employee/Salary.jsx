...
const Salary = () => {
  ...
  const downloadPDF = (item) => {
    ...
    doc.text(`Amount (${currency})`, 60, y + 6);
    ...
    drawRow("Basic Salary", `${currency}${item.basic_salary}`, "Tax", `${currency}${item.deductions.tax_amount}`);
    drawRow("Gross Salary", `${currency}${item.salary_breakdown.gross_salary}`, "PF", `${currency}${item.deductions.pf_amount}`);
    drawRow("", "", "Leave Deduction", `${currency}${Number(item.deductions.leave_deduction).toFixed(2)}`);
    drawRow("", "", "Total Deduction", `${currency}${Number(item.deductions.total_deduction).toFixed(2)}`);
    ...
    doc.text(`Net Salary: ${currency}${Number(item.salary_breakdown.net_salary).toFixed(2)}`, 12, y + 7);
                  ...
                  <div className="bg-gray-50 px-6 pb-6 pt-2 rounded-b-3xl">
                    <div className="space-y-4 text-gray-700 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-right font-medium">RS{item.basic_salary}</div>
                      </div>
